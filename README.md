...
- **Memory hints** — every word ships with a built-in memory hook (a cognate, a root breakdown or a sound-alike) and a symbol, so "Show hint" always has something to reveal. On top of that you can type your own reminder and attach a picture, either from the word list or straight from the card mid-session. It stays hidden while you study until you tap "Show hint". Photos are downscaled and kept in IndexedDB on the device.
