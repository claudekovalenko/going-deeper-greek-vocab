...
- **Memory hints** — every word ships with a built-in memory hook (a cognate, a root breakdown or a sound-alike) and a symbol, so "Show hint" always has something to reveal. Every hook is editable: the field is seeded with the built-in text, you rewrite it into whatever makes it stick, and "Restore the original hook" brings mine back. You can attach a picture too, either from the word list or straight from the card mid-session. It stays hidden while you study until you tap "Show hint". Photos are downscaled and kept in IndexedDB on the device.
