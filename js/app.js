...
function mountHint(w, allowAdd = true) {
  ...
  const draw = () => {
    ...
    const text = hookText(w);
    body.innerHTML = `
      ${url ? `<img class="hint-img" alt="Your picture for ${esc(w.g)}" src="${url}">` : ""}
      ${text ? `<p class="hint-note">${w.icon && !mine ? `<span class="hint-icon">${w.icon}</span>` : ""}${esc(text)}</p>` : ""}
      ${allowAdd ? `
        <div class="hint-tools">
          <label class="tool-label" for="my-note">Memory hook — edit it into whatever makes it stick</label>
          <textarea id="my-note" class="hook-input" rows="3"
            placeholder="Sounds like…">${esc(text)}</textarea>
          <div class="hint-tool-row">
            <button class="btn secondary btn-inline" id="save-note">Save hook</button>
            <button class="btn secondary btn-inline" id="pick-pic">${url ? "Change picture" : "Add a picture"}</button>
          </div>
          ${mine && w.mn ? `<button class="btn secondary btn-inline wide" id="reset-hook">Restore the original hook</button>` : ""}
          <div id="note-fb"></div>
        </div>` : ""}`;
