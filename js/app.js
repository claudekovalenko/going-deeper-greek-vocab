...
async function askClaude(prompt) {
  ...
  let body = {};
  try { body = await res.json(); } catch { /* leave it empty; handled below */ }
  if (!res.ok) throw new Error(body.error || `The endpoint returned ${res.status}.`);
  if (!body.text) throw new Error("The endpoint sent nothing back.");
  return body.text;
}
